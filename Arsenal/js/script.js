...
var phoneMask = IMask(
    document.querySelector('#modalPhone'), {
      mask: '+{7}(000)000-00-00'
});

var phoneMaskLead = IMask(
    document.querySelector('#leadPhone'), {
      mask: '+{7}(000)000-00-00'
});

document.querySelectorAll('.leadForm').forEach(e => {
    e.addEventListener('submit', (e) => {
        e.preventDefault()

        fetch('/').then(function() {
            document.querySelector('.leadModal .btn-close').click()

            let thanksModalActive = new bootstrap.Modal(document.getElementById('thanksModal'), {
                keyboard: false
            })
            
            thanksModalActive.show()
        })
        .catch(function() {
            console.log('fail')
        });
